...
DATA_FILENAME = "sleepsense_feature_engineered.csv"
DATA_PATH_CANDIDATES = [
    ROOT_DIR / "data" / "processed" / DATA_FILENAME,
    ROOT_DIR / DATA_FILENAME,
    Path.cwd() / "data" / "processed" / DATA_FILENAME,
    Path.cwd() / DATA_FILENAME,
    Path.cwd() / "capstone" / "data" / "processed" / DATA_FILENAME,
    Path.cwd() / "capstone" / DATA_FILENAME,
]
...
def load_data() -> pd.DataFrame:
    ...
    st.error(
        "Dataset dashboard tidak ditemukan. Pastikan file "
        f"`data/processed/{DATA_FILENAME}` ikut di-upload ke repository GitHub."
    )
    st.markdown(f"Path yang dicek:\n\n{searched_paths}")
    st.stop()
